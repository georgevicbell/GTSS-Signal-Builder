...
const approach = (signalId: string, streetName: string, compassBearing: number | null): Approach => ({
...
describe('GTSS utilities', () => {
  it('merges class names and derives unique street names', () => {
    ...
    expect(getDerivedStreetNames('SIG-1', approaches)).toEqual({ streetName1: 'Main Street', streetName2: 'First Avenue' });
    expect(getSignalDisplayName(signal('SIG-1'), approaches)).toBe('SIG-1 - Main Street & First Avenue');
    expect(getSignalDisplayName(signal('SIG-2'), approaches)).toBe('SIG-2 - Fallback One & Fallback Two');
  });

  it('suggests a nearby same-direction street and ignores unsuitable approaches', () => {
    const signals = [signal('CURRENT'), signal('NEARBY', 47.611, -122.33), signal('FAR', 48, -122.33)];
    const approaches = [
    ...
    expect(suggestStreetNameForApproach({
      bearing: 0,
      signalLat: 47.61,
      signalLng: -122.33,
