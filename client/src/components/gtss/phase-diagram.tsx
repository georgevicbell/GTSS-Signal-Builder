...
interface PhaseDiagramProps {
  ...
  /** Signal ID, used to look up the agency's left-hand-traffic setting. */
  signalId?: string;
...
export default function PhaseDiagram({
  ...
  const isLht = isLhtForSignalId(signalId);
  ...
  const getPhaseOffset = (phase: PhaseData): number => {
    ...
    // LHT mirrors which side of the centerline a movement's lane sits on.
  ...
  return (
    <svg ref={svgRef} viewBox="-20 0 340 360" className="w-full h-full">
      ...
      <g transform={intersectionName && !compact ? "translate(0, 20)" : ""}>
        ...
        {phases
          .filter((p) => p.isPedestrian || p.movementType === "Pedestrian")
          .map((phase, idx) => renderPedestrianLine(phase, idx))}
